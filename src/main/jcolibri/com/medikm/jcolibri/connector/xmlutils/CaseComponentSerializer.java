 ...
 * 04/11/2007
 */
package com.medikm.jcolibri.connector.xmlutils;
...
import com.medikm.jcolibri.connector.plaintextutils.PlainTextTypeConverter;
import com.medikm.jcolibri.util.AttributeUtils;
...
/**
 * Utility class to serialize CaseComponents into xml files. This class will be
 * used in future versions to implement the xml connector.
 * 
		...
		try {
			...
			for (int i = 0; i < nl.getLength(); i++) {
				...
				if (nodeName.equals("CaseComponent")) {
					Attribute at = new Attribute(child.getAttributes().getNamedItem("Name").getTextContent(),
							Class.forName(child.getAttributes().getNamedItem("Class").getTextContent()));
		...
		try {
			...
			for (Attribute at : AttributeUtils.getAttributes(casecomponent)) {
				if (CaseComponent.class.isAssignableFrom(at.getType()))
					root.appendChild(
							serializeCaseComponent((CaseComponent) at.getValue(casecomponent), at.getName(), doc));
				else {
