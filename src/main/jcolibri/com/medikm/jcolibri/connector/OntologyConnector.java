 ...
 * 07/06/2007
 */
package com.medikm.jcolibri.connector;
...
import com.medikm.jcolibri.connector.ontologyutils.OntologyInfo;
import com.medikm.jcolibri.connector.ontologyutils.OntologyMapping;
import com.medikm.jcolibri.datatypes.Instance;
import com.medikm.jcolibri.exception.InitializingException;
import com.medikm.jcolibri.util.FileIO;
import com.medikm.jcolibri.util.ProgressController;
...
/**
 * Implements a generic Ontology connector. It uses OntoBridge to manage the
 * ontologies and the reasoner. To configure this connector create a
 * configuration xml file following this schema: <a href=
 * "OntologyConnector.xsd">/doc/configfilesSchemas/OntologyConnector.xsd</a>:
 * <p>
 * <img src="OntologyConnectorSchema.jpg">
 * <p>
 * This connector only maps case structures without compound attributes. All
 * attributes must be Instance typed.
 * 
...
public class OntologyConnector implements Connector {
	...
	/**
	 * Initializes the connector from an XML config file. This method reads the
	 * configuration and launches OntoBridge with the Pellet reasoner. Then the
	 * ontologies are loaded into memory.
	 * 
	...
	public void initFromXMLfile(URL file) throws InitializingException {
		...
		try {
			...
			OntoBridge ob = com.medikm.jcolibri.util.OntoBridgeSingleton.getOntoBridge();
	...
	private void retrieveCaseComponent(OntoBridge ob, CaseComponent cc, String mainInstanceName,
			ArrayList<OntologyMapping> mappings) throws Exception {
		// Id
	...
	public Collection<CBRCase> retrieveSomeCases(CaseBaseFilter filter) {
		org.apache.commons.logging.LogFactory.getLog(this.getClass())
				.error("retrieveSomeCases(CaseBaseFilter) method is not yet implemented");
		return null;
	}
	...
	public void storeCases(Collection<CBRCase> cases) {
		...
		for (CBRCase _case : cases) {
			try {
				...
				org.apache.commons.logging.LogFactory.getLog(this.getClass())
						.error("Error storing case: " + _case + ". Cause: " + e.getMessage());
			}
			ProgressController.step(this.getClass());
		}
		ProgressController.finish(this.getClass());
	...
	/**
	 * If there was any modification to the ontology, the owl file is replaced
	 * with a new one that contains the changes. The new owl file is completely
	 * regenerated from scrach with the current content of the reasoner (not
	 * including the inferred model). OntoBridge uses the RDF/XML-ABBREV syntax
	 * for the owl files.
	 * 
	...
	/**
	 * Deletes cases in the ontology. Only the main instance (case id mapped
	 * instance) is removed, so the instances mapped to attributes are keep.
	 * 
	...
	public Collection<CBRCase> retrieveAllCases(String condition) {
		return null;
	}
