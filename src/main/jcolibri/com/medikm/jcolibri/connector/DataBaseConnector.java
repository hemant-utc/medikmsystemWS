...
/**
 * Implements a data base connector using the
 * <a href="www.hibernate.org">Hibernate package</a>.
 * <p>
 * The configuration file follows the schema defined in <a href=
 * "DataBaseConnector.xsd">/doc/configfilesSchemas/DataBaseConnector.xsd</a>:
 * <p>
 * <img src="DataBaseConnectorSchema.jpg">
 * <p>
 * There are several examples that incrementally show how to use this connector:
...
public class DataBaseConnector implements Connector {
		...
		try {
			...
			try {
				...
				org.apache.commons.logging.LogFactory.getLog(this.getClass())
						.info("Case does not have justification of the solution");
			}
	...
	public Collection<CBRCase> retrieveAllCases(String condition) {
		...
		try {
			...
			session = sessionFactory.openSession();
			transaction = session.beginTransaction();
			descList = session.createQuery("from " + descriptionClassName + condition).list();
			transaction.commit();
			session.close();
		...
		org.apache.commons.logging.LogFactory.getLog(this.getClass())
				.info(res.size() + " cases read from the database.");
		return res;
	}
	...
	public void storeCases(Collection<CBRCase> cases) {
		...
		org.apache.commons.logging.LogFactory.getLog(this.getClass())
				.info(cases.size() + " cases stored into the database.");
