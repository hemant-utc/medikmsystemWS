...
import com.medikm.dto.TbDiscussionForum;

/**
 * A data access object (DAO) providing persistence and search support for
 * TbDiscussionForum entities. Transaction control of the save(), update() and
 * delete() operations can directly support Spring container-managed
 * transactions or they can be augmented to handle user-managed Spring
 * transactions. Each of these methods provides additional information for how
 * to configure it for the desired type of transaction control.
 * 
 * @see com.medikm.dto.TbDiscussionForum
 * @author MyEclipse Persistence Tools
 */
@SuppressWarnings({ "rawtypes", "deprecation" })
public class TbDiscussionForumDAO extends BaseHibernateDAO {
	private static final Log log = LogFactory.getLog(TbDiscussionForumDAO.class);
	// property constants
	...
	public void save(TbDiscussionForum transientInstance) {
		log.debug("saving TbDiscussionForum instance");
		try {
	...
	public void delete(TbDiscussionForum persistentInstance) {
		log.debug("deleting TbDiscussionForum instance");
		try {
	...
	public TbDiscussionForum findById(java.lang.Integer id) {
		log.debug("getting TbDiscussionForum instance with id: " + id);
		try {
			TbDiscussionForum instance = (TbDiscussionForum) getSession().get("com.medikm.dto.TbDiscussionForum", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(TbDiscussionForum instance) {
		log.debug("finding TbDiscussionForum instance by example");
		try {
			List results = getSession().createCriteria("com.medikm.dto.TbDiscussionForum").add(Example.create(instance))
					.list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding TbDiscussionForum instance with property: " + propertyName + ", value: " + value);
		try {
			String queryString = "from TbDiscussionForum as model where model." + propertyName + "= ?";
	...
	public List findAll() {
		log.debug("finding all TbDiscussionForum instances");
		try {
			String queryString = "from TbDiscussionForum";
			Query queryObject = getSession().createQuery(queryString);
			return queryObject.list();
		} catch (RuntimeException re) {
	...
	public TbDiscussionForum merge(TbDiscussionForum detachedInstance) {
		log.debug("merging TbDiscussionForum instance");
		try {
			TbDiscussionForum result = (TbDiscussionForum) getSession().merge(detachedInstance);
	...
	public void attachDirty(TbDiscussionForum instance) {
		log.debug("attaching dirty TbDiscussionForum instance");
		try {
	...
	public void attachClean(TbDiscussionForum instance) {
		log.debug("attaching clean TbDiscussionForum instance");
		try {
